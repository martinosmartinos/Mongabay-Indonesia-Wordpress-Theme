...
<channel>
	...
	<item>
		...
		?>
		<?php if($byline && empty($author)) :?>
		<dc:creator><![CDATA[<?php $bylines = wp_get_object_terms($post->ID, 'byline'); foreach ($bylines as $byline) {$byline_name = $byline -> name; echo $byline_name;} ?>]]></dc:creator>
		<?php endif; ?>
		<?php if($author && empty($byline)) :?>
		<dc:creator><![CDATA[<?php $author = get_post_meta( $post_id, 'author', true); echo $author ?>]]></dc:creator>
		<?php endif; ?>
